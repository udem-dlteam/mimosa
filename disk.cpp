...
uint32 disk_BIOS_CHS_to_LBA (disk* d, uint8 BIOS_CHS[3])
{
  ...
  return (((CAST(uint32,(C << shift)) * d->_.ide.dev->heads_per_cylinder) + H)
...
uint32 disk_max_BIOS_CHS_to_LBA (disk* d)
{
  ...
  return ((cyls
           * d->_.ide.dev->heads_per_cylinder
           * d->_.ide.dev->sectors_per_track) << shift) - 1;
}
...
static native_string partition_name_from_type (uint8 type)
{
  ...
  for (i=0;
       i<sizeof (partition_type_table) / sizeof (partition_type_table[0]);
       i++)
  ...
  for (i=0; i<DISK_CACHE_SIZE; i++)
    {
      ...
      if (cb == NULL)
        fatal_error ("can't allocate disk cache");
