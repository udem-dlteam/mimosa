...
static error_code open_root_dir(fat_file_system* fs, fat_file** result) {
  fat_file* f = CAST(fat_file*, kmalloc(sizeof(file)));
...
error_code _file_set_pos_from_start(fat_file* f, uint32 position);
  ...
  while (path[0] != '\0') {
    if (path[0] == '/') {
      ...
      while (path[0] != '\0' && path[0] != '/') {
        if (i >= FAT_NAME_MAX) return FNF_ERROR;
...
static error_code fat_fetch_entry(fat_file_system* fs, fat_file* parent, native_char* name, fat_file** result) {
  int count = 0;
  int found_count = 0;
  ...
  while ((err = fat_read_file(parent, &de, sizeof(de))) == sizeof(de)) {
    ...
    if (de.DIR_Name[0] != 0xe5 && (de.DIR_Attr & FAT_ATTR_HIDDEN) == 0 &&
      ...
      if (i == FAT_NAME_LENGTH) {
        ...
        f = CAST(fat_file*, kmalloc(sizeof(file)));
...
static error_code find_first_empty_FAT_cluster(fat_file_system* fs,
                                               uint32* cluster) {
  if (fs->kind != FAT32_FS) {
    return UNIMPL_ERROR;
  } else {
...
static error_code get_fat_link_value(fat_file_system* fs, uint32 cluster,
                                     uint32* value) {
  if (fs->kind != FAT32_FS) {
    return UNIMPL_ERROR;
  } else {
...
static error_code set_fat_link_value(fat_file_system* fs, uint32 cluster,
                                     uint32 value) {
  if (fs->kind != FAT32_FS) {
    return UNIMPL_ERROR;
  } else {
...
static error_code update_file_length(fat_file* f) {
  ...
  fat_file* parent_dir = CAST(fat_file*, kmalloc(sizeof(file)));
...
static error_code unlink_file(fat_file* f) {
  error_code err = NO_ERROR;
...
static error_code truncate_file(fat_file* f) {
  error_code err = NO_ERROR;
...
error_code open_file(native_string path, native_string mode, fat_file** result) {
  error_code err = NO_ERROR;
  fat_file_system* fs;
  file_mode md;
  native_char normalized_path[FAT_NAME_MAX + 1];
...
error_code close_file(fat_file* f) {
  kfree(f);
  return NO_ERROR;
}
...
error_code write_file(fat_file* f, void* buff, uint32 count) {
  error_code err = NO_ERROR;
  fat_file_system* fs = f->fs;
...
error_code read_file(fat_file* f, void* buf, uint32 count) {
  if (count > 0) {
    fat_file_system* fs = f->fs;
    error_code err;
...
void file_reset_cursor(fat_file* f) {
  fat_file_system* fs = f->fs;
...
error_code file_set_pos_from_start(fat_file* f, uint32 position) {
...
erro_code file_move_cursor(fat_file* f, int32 n) {
  uint32 displacement;
  BIOS_Parameter_Block* p;
  cache_block* cb;
  error_code err = NO_ERROR;
...
error_code file_set_to_absolute_position(fat_file* f, uint32 position) {
