...
static error_code
fat_fetch_first_empty_directory_position(fat_file *directory, uint32 *position,
...
static void unpack_fat_time(uint16 fat_time, uint8 *hours, uint8 *minutes,
                            uint8 *seconds) {
  // According to the FAT specification, the FAT time is set that way:
  // Bits 15-11: the hour
  // Bits 10-5 : the minutes
...
static void unpack_fat_date(uint16 fat_date, int16 *year, uint8 *month,
                            uint8 *day) {
  // According to the FAT specification, the FAT date is set that way:
  // Bits 15-9: Year relative to 1980
  // Bits 8-5: Month
...
static error_code fat_open_directory_entry(fat_file *f,
  ...
  if (ERROR(err = new_fat_file(&parent_dir)))
    return err;
...
static error_code fat_move_cursor(file *ff, int32 n) {
  ...
  if (n < 0) {
    ...
    // If we cross a section boundary, the design of the FAT FS requires to
    ...
    // Forward mvmt
    displacement = n;
    bool crosses_section_boundary =
        ((displacement + f->current_section_pos) >= f->current_section_length);
      ...
      if (f->current_section_pos >= f->current_section_length) {
        if (ERROR(err = next_FAT_section(f))) {
          if (err != EOF_ERROR) {
            ...
            if (ERROR(err = fat_32_set_fat_link_value(fs, f->current_cluster,
                                                      cluster))) {
              return err;
            }

            if (ERROR(err =
                          fat_32_set_fat_link_value(fs, cluster, FAT_32_EOF))) {
              return err;
            }
            ...
            if (ERROR(err = next_FAT_section(f))) {
              if (err == EOF_ERROR) {
                panic(L"Failed to allocate a new FAT cluster, but no error was "
                      L"returned");
              } else {
                return err;
              }
    ...
    if (de.DIR_Name[0] != FAT_UNUSED_ENTRY &&
      ...
      if (name_len <= FAT_NAME_LENGTH) {
        for (i = 0; i < FAT_NAME_LENGTH; i++) {
          if (de.DIR_Name[i] != sfn_buff.name[i])
            break;
        }
...
error_code fat_32_get_fat_link_value(fat_file_system *fs, uint32 cluster,
  ...
  {
    if (ERROR(err = disk_cache_block_acquire(d, lba, &cb)))
      return err;
    rwmutex_readlock(cb->mut);
...
*/
static error_code
fat_fetch_first_empty_directory_position(fat_file *directory, uint32 *_position,
                                         uint8 required_spots) {
  FAT_directory_entry de;
  ...
  while (((err = fat_read_file(CAST(file *, directory), &de, sizeof(de))) ==
          sizeof(de))) {
...
static error_code fat_allocate_directory_entry(fat_file_system *fs,
  ...
  for (uint8 entry_idx = 0; entry_idx < (required_spots - 1); ++entry_idx) {
    ...
    if (0 == entry_idx) {
      ...
      // Required spots - 2 because one is the directory entry and we don't
      // count the current long name entry (we want the number of chars in this
      // specific entry)
      max_chars = chars =
...
static error_code fat_32_create_empty_file(fat_file_system *fs,
                                           fat_file *parent_folder,
                                           FAT_directory_entry *de,
                                           native_char *name, uint8 attributes,
                                           fat_file **result) {
  error_code err;
  fat_file *f = NULL;
  ...
  // -------------------------------------------------------------------------------
  // Write the directory entry to the disk, modifications must be done by this
  // point
  // -------------------------------------------------------------------------------
  ...
  uint32 position;
  if (ERROR(err = fat_allocate_directory_entry(fs, parent_folder, de, name,
                                               &position))) {
    return err;
  }
  ...
  uint8 name_len = kstrlen(name);
  f->header.name =
      CAST(native_string, kmalloc(sizeof(native_char) * (name_len + 1)));
  memcpy(f->header.name, name, name_len + 1);
...
static error_code fat_create_file(native_char *name, fat_file *parent_folder,
                                  fat_file **result) {
  error_code err = NO_ERROR;
  FAT_directory_entry de;
  fat_file_system *fs =
      CAST(fat_file_system *, parent_folder->header._fs_header);
...
/*
From the "_part" string (that represents a decomposed file path), fetch the
parent of the last item in the path (wheter it is a folder or a file). The depth
represents the number of levels in the path. The result is placed into result
and an error code is returned. Exemple: File path:
//parentA//parentB//parentC//parentD//file\0 _parts:
\0parentA\0parentB\0parentC\0parentD\0file\0 Depth: 5 The last file is file, so
the parent returned is parentD. For a depth of 4, the last file would be
parentD, so the parent returned would be parentC. This function is expected to
move the _parts "cursor" forward up until the beginning of the last part of the
name. In the first example, the _parts pointer would be: file\0 at the end of
the function call.
*/
...
static error_code fat_mkdir(fs_header *ffs, native_string name, uint8 depth,
                            file **result) {
  error_code err = NO_ERROR;
  fat_file_system *fs = CAST(fat_file_system *, ffs);
  ...
  if (HAS_NO_ERROR(err)) {
    ...
    if (ERROR(err =
                  file_write(folder, &file_de, sizeof(FAT_directory_entry)))) {
      // error
    } else if (ERROR(err = file_write(folder, &dot_dot_entry,
                                      sizeof(FAT_directory_entry)))) {
      // error
    }
  }
...
static error_code fat_unlink_file(fat_file *f) {
  ...
  uint32 next_clus;
  do {
    if (ERROR(err = fat_32_get_fat_link_value(
                  CAST(fat_file_system *, f->header._fs_header), cluster,
                  &next_clus)))
      break;
    if (ERROR(
            err = fat_32_set_fat_link_value(
                CAST(fat_file_system *, f->header._fs_header), cluster, NULL)))
      break;
...
static error_code fat_truncate_file(fat_file *f) {
  ...
  if (ERROR(err = fat_unlink_file(f)))
    return err;
  uint32 clus = f->first_cluster;

  if (ERROR(
          err = fat_32_set_fat_link_value(
              CAST(fat_file_system *, f->header._fs_header), clus, FAT_32_EOF)))
    return err;

  f->length = 0;
  if (ERROR(err = fat_update_file_length(f)))
    return err;

  return err;
}

error_code fat_file_open(fs_header *ffs, native_string parts, uint8 depth,
      ...
      if (ERROR(err)) {
        if (FNF_ERROR == err) {
          ...
          break;
                           ...
                           CAST(file *, child),
                           (child->length > 0) ? (child->length - 1) : 0))) {
        break;
      }
...
/*
Read the LFN that starts at the cluster "cluster" in the position "position",
where the cluster indicates the start of a directory and the position is the
absolute position relative to that cluster.
...
static error_code read_lfn(fs_header *fs, uint32 cluster, uint32 position,
  ...
  if (ERROR(err = new_fat_file(&reader))) {
    return err;
  }
  ...
  while (HAS_NO_ERROR(err)) {
    ...
    position = position - sizeof(long_file_name_entry);
    ...
    lfn_entry_count++;
    if (lfn_e.LDIR_ord & FAT_LAST_LONG_ENTRY) {
      break;
    }
  }
...
static dirent *fat_readdir(DIR *dir) {
  ...
  case FAT32_FS: {
    ...
    while ((err = fat_read_file(CAST(file *, f), &de, sizeof(de))) ==
           sizeof(de)) {
      if (de.DIR_Attr == FAT_ATTR_LONG_NAME)
        continue;
      if (de.DIR_Name[0] == 0)
        break;
      if (de.DIR_Name[0] != FAT_UNUSED_ENTRY) {
        if ((de.DIR_Attr & FAT_ATTR_HIDDEN) == 0 &&
          ...
          error_code lfn_err = read_lfn(f->header._fs_header, f->first_cluster,
...
static fat_open_chain *new_chain_link(fat_file_system *fs, fat_file *file) {
  ...
  if (NULL == (nlink = CAST(fat_open_chain *, kmalloc(sizeof(fat_open_chain)))))
    return NULL;
...
static error_code fat_stat(fs_header *header, file *ff, stat_buff *buf) {
  ...
  unpack_fat_time(fat_creation_time, &creation_hours, &creation_minutes,
                  &creation_seconds);
  unpack_fat_time(fat_modification_time, &modif_hours, &modif_minutes,
                  &modif_seconds);

  int16 creation_years;
  uint8 creation_month, creation_day;
  int16 modif_year;
  uint8 modif_month, modif_day;

  unpack_fat_date(fat_creation_date, &creation_years, &creation_month,
                  &creation_day);
  unpack_fat_date(fat_modification_date, &modif_year, &modif_month, &modif_day);

  uint32 creation_secs_from_date =
      days_from_civil(creation_years, creation_month, creation_day) *
      day_in_sec;
  uint32 modif_secs_from_date =
      days_from_civil(modif_year, modif_month, modif_day) * day_in_sec;
...
error_code mount_fat(vfnode *parent) {
  ...
  _fat_file_vtable._file_set_to_absolute_position =
      fat_set_to_absolute_position;
